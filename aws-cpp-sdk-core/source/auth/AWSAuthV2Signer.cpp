...
bool AWSAuthV2Signer::Sign(Aws::Http::HttpRequest& request, long long expires) const
{
    ...
    {
        signingStringStream << request.GetHeaderValue(Aws::Http::CONTENT_MD5_HEADER) << NEWLINE;
    }
    else
    {
    ...
    {
        signingStringStream << request.GetHeaderValue(Aws::Http::CONTENT_TYPE_HEADER) << NEWLINE;
    }
    else
    {
        signingStringStream << NEWLINE;
    ...
    for (const auto& header : CanonicalizeHeaders(request.GetHeaders()))
    {
        ...
        {
            signingStringStream << header.first.c_str() << ":" << header.second.c_str() << NEWLINE;
        }
    }
    ...
    auto hashResult = m_HMAC->Calculate(ByteBuffer((unsigned char*)signingString.c_str(), signingString.length()),
                                       ByteBuffer((unsigned char*)credentials.GetAWSSecretKey().c_str(),
                                                  credentials.GetAWSSecretKey().length()));
    if (!hashResult.IsSuccess())
    {
        AWS_LOGSTREAM_ERROR(v2LogTag, "Failed to hash (sha1) request string \"" << signingString << "\"");
        return false;
    }
    ...
    Aws::StringStream ss;
    ss << "AWS" << " " << credentials.GetAWSAccessKeyId() << ":" << signature;
...
bool AWSAuthV2Signer::PresignRequest(Aws::Http::HttpRequest& request, const char*, const char*, long long expirationTimeInSeconds) const
{
    long long epoch = (long long)(GetSigningTimestamp().SecondsWithMSPrecision()) + expirationTimeInSeconds;
    ...
    AWS_LOGSTREAM_DEBUG(v2LogTag, "Presigned request: " << request.GetURIString(true));
